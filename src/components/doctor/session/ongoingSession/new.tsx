...
const PatientRecordEntryForm = ({ onSubmit }: { onSubmit: (data: any) => void }) => {
  ...
  const handleDiagnosisInputConfirm = () => {
    if (diagnosisInputValue && !diagnosisCategories.includes(diagnosisInputValue)) {
      ...
      <div className="flex flex-col border p-4">
        ...
        <div className="grid justify-items-end text-[#FFFFFF] m-4">
          <div>
            ...
            <button type="submit" className="bg-[#FF7300] p-3 w-56 rounded-[16px]">
              Continue
            </button>
          </div>
        </div>
